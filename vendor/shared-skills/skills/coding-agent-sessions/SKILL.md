...
If scope is broad (multi-month, many repos/platforms, or a vague "what happened with X"), split independent searches by names/errors, repos/cwds, platforms/models, and time windows. Use `AgentSwarm` (with a prompt template containing `coding-agent-sessions`) or sequential `Agent` calls for these lanes when delegation is useful; otherwise run the finder calls in parallel. Merge candidates by `id`/`path`, then read the most likely sessions.
...
- **Finder execution** — The bundled `scripts/find-agent-sessions.py` is invoked through the `Bash` tool (`python3 scripts/find-agent-sessions.py ...`). Use `python` if `python3` is unavailable. If the script is not present in this distribution, fall back to `Bash`/`Read`/`Grep` against the known platform stores listed in `references/all-platforms.md`.
...
- **Dropped Codex runtime concepts** — `codex_app.*` APIs, `multi_agent_v2`, thread titles, `set_thread_title`, and `lazycodex-gate-reviewer` agent types do not exist in Kimi Code CLI; this skill uses `Bash` for the finder and `Agent`/`AgentSwarm` only for parallel lane delegation.
