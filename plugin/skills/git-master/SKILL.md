...
- Use `Bash` for all git commands and repository inspection.
- Use `Agent(prompt=..., subagent_type="coder"|"explore"|"plan")` for large history investigations, multi-file refactors across commits, or when you need a subordinate reasoning pass.
- Use `AgentSwarm` with a prompt template referencing `git-master` when you need parallel independent investigations (for example, tracing several files or candidate commits at once).
- Use `Write` / `Edit` for any file changes; never apply patches through raw shell.
- Kimi has no thread-title or per-skill agent TOML concepts; drop any `codex_app.set_thread_title`, agent YAML, or Codex-runtime-specific instructions.
- Kimi Code CLI has no built-in browser. Browser-dependent steps (e.g., GitHub attachment upload flows) should use the `kimi-webbridge` skill if available, or be handed off to the user with exact instructions.
