...
本仓库定位为 **Kimi Code CLI 版的 OmO 分发版**，目标是把 LazyCodex（Codex CLI 插件）的能力移植到 Kimi Code CLI。当前代码已经搭起整体骨架，核心组件 CLI、MCP、Skill、安装器均已存在，测试（39 个测试文件 / 223 个测试）与构建均能通过。近期修复轮次已关闭大部分 P0/P1 缺口，现状为：
