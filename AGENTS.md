...
| `codegraph` | `SessionStart`, `PostToolUse` | Structural code search MCP (TS/JS/Python/Go/Rust indexer) |
...
| `lsp` | `PostToolUse`, `PostCompact` | Real LSP client MCP (`lsp_status`, `lsp_diagnostics`, `lsp_goto_definition`, `lsp_find_references`) |
...
| `teammode` | Skill-driven | Parallel multi-agent state script (`init`, `add-member`, `status`, `archive`, `delete`) |
